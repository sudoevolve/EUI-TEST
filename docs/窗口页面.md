...
- `frameRateLimit`：可选动态帧率上限回调。返回 `0` 表示不限制，返回 `60` 表示最多 60 FPS。
...
double appFrameRateLimit() {
    return limit60 ? 60.0 : 0.0;
}
}
...
`gallery` 当前默认开启 60 FPS 限制，默认关闭 Glass/blur。
...
- 如果 `app::frameRateLimit()` 返回正数，则动画帧率上限为 `min(refreshRate, frameRateLimit)`。
