...
当前只封装鼠标左键 pointer 事件，还没有键盘、滚轮、右键、多指针和 IME 输入。
...
2. 从 DSL 树中找出最上层命中的 interactive 元素。
3. 如果已有 active 捕获元素，则优先使用捕获元素。
4. 更新每个 interactive 元素的 `InteractionState`。
5. 根据 hover 设置 GLFW 手型 cursor。
6. 如果产生 `clicked`，执行 `onClick`，并设置 `needsCompose()`。
7. Rect 根据交互状态推进 hover / press 缓动并标记 dirty rect。
...
- 没有 focus manager。
- 没有事件冒泡和捕获阶段。
- 没有公开 hover / drag 回调，当前公开回调只有 click。
